...
Equation \@ref(eq:uni-irt) shows what is known as the 3 parameter logistic (3PL) item response theory model. In addition to $\theta$, which represents the respondents ability, there are $a$, $b$, and $c$ parameters for each item. This unidimensional model gives rise to an item characteristic curve for each item (for example, Figure \@ref(fig:icc)).
...
In equation \@ref(eq:ncmp-mirt), $L$ refers to the number of dimensions measured by the item (two in our math word problem example). Because the probability from each dimension is multiplied by the others, a low probability on any dimension will result in a low probability overall. Thus, a respondent needs to have a high ability on all dimensions, as other dimensions cannot compensate for the lack of another.
